...
for (0 => int i; i < durs.size(); i++) {
    if (melody[i] != 0 && melody[i] != -1) {
        sendOutMIDInote(1,melody[i], 70);
        durs[i] => now;
        
        sendOutMIDInote(0, melody[i], 70);
        0.00001::second => now;
    }

    if (melody[i] == -1) {
            sendOutMIDInote(1,chords[j][0], 70);
            sendOutMIDInote(1,chords[j][1], 70);
            sendOutMIDInote(1,chords[j][2], 70);
            durs[i] => now;
        
            sendOutMIDInote(0,chords[j][0], 70);
            sendOutMIDInote(0,chords[j][1], 70);
            sendOutMIDInote(0,chords[j][2], 70);
            0.00001::second => now;
            j+1 => j;
     }
     if(melody[i] == 0) {
